...
torch.save({
    'epoch': epochs, # 训练轮次
    'model_state_dict': model.state_dict(), # 参数权重
    'optimizer_state_dict': optimizer.state_dict(), # 优化器状态
    'loss': loss.item()
},'gru_chaos_checkpoint.pth') # 模型保存

checkpoint = torch.load('gru_chaos_checkpoint.pth', map_location=device)
model_loaded = GRUChaos().to(device)
optimizer_loaded = torch.optim.Adam(model_loaded.parameters(),lr=1e-4)
