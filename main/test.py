...
# GPU 版 Arnold 变换（单通道）
# ---------------------------
def arnold_transform_channel_tensor(channel, iterations=20, a=1, b=1, device=None):
...
def inverse_arnold_transform_channel_tensor(channel, iterations=20, a=1, b=1, device=None):
    """逆 Arnold 变换"""
    if device is None:
        device = channel.device
...
def chaotic_diffusion_rgb_tensor(img_rgb, kr, kg, kb, device=None):
    ...
    cipher = cipher ^ k.to(torch.uint8) # 再次异或 Ci' = Ci XOR ki
...
def inverse_chaotic_diffusion_rgb_tensor(cipher_rgb, kr, kg, kb, device=None):
    ...
    # 先异或
    flat = (flat ^ k.to(torch.uint8)).to(torch.int16)


    # tmp[0]=cipher[0]; tmp[i]=cipher[i]-cipher[i-1]
    # 使用整数并加上256再模
